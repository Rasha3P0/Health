...
| 8d | Medication (multi-select incl. "prescribed privately") and supplements / over-the-counter | `prep.ts` → `MEDS`, `SUPPLEMENTS`; `letter.ts` | Is the private-prescription line accurate? Informational only: never comments on interactions or suitability. | Draft |
| 8e | Day to day (exercise, alcohol, smoking or vaping, caffeine, meals) | `prep.ts` → `LIFESTYLE` | Check the 14-units wording against current NHS guidance. Weight, BMI, calories and food diaries are deliberately not asked (disordered-eating risk). Owner to decide whether Meals stays. | Draft |
